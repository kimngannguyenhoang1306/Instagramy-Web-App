...
class Parser(HTMLParser):
    ...
    def handle_data(self, data):
        if data.startswith("window._sharedData"):
            try:
                self.Data2 = (json.loads(data[data.find('{"config"'): -1]))
                self.Data = self.Data2
                '''check = self.Data["entry_data"]["TagPage"][0]["data"]["top"]["sections"]
                while (True):
        ...
        if "window.__additionalDataLoaded(" in data and "graphql" not in self.Data2:
            parameters = data.split("window.__additionalDataLoaded(")[1].split(");</script>")[0]
            if parameters and "," in parameters:
                shared_data = parameters.split(",", 1)[1]
                if shared_data:
                    self.Data1 = (json.loads(shared_data[:-2]))
                    self.Data["entry_data"]["PostPage"][0].update(self.Data1)
...
class UserParser:
    ...
    def posts_display_urls(self) -> list:
        ...
        return [i[8] for i in self.posts]
...
class TagParser:
    ...
    def posts_display_urls(self) -> list:
        ...
        """
        return [i[8] for i in self.top_posts]
